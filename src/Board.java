...
public class Board implements ActionListener{
    ...
    private JButton buttons[] = new JButton[9]; // Creates an array of 9 buttons that serve as our input for player and output for both computer and player
    ...
    public char[] boardState() // converts the strings of the button texts to characters for easy processing by the computer
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            if (buttons[i].getText().equals(AI))
                ...
                result[i] = '#'; // stores any empty values as # symbols
        }
        return result;
    }
    ...
    public int determine(char[] current) // returns the index of the best possible move by the computer
    {
        ...
        for (int move : moves)
        {
            ...
            current[move] = '#'; // Resets the grid so that it does not consider possible move in future
    ...
    public int min(char[] current)  // Returns the score of the best possible move made by the player given a map of the grid
    {
        ...
        for (int move : moves)
        {
            ...
            current[move] = '#'; // Resets the grid for next simulated move by the player
    ...
    private ArrayList<Integer> legal(char[] current) // Returns an ArrayList of all the indexes of legal moves
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            if (current[i] == '#')    // checks if the space is empty
            {
                result.add(i);
            }
        }
